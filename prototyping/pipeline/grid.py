...
def find_tile_centers(warped, debug=False):
    ...
    # The warp_board() call expands corners by WARP_PAD_PCT, so the outermost
    # WARP_PAD_PCT fraction of the warped image is background/frame, not tiles.
    # Peaks found there are artifacts that create phantom rows/columns.
    m = WARP_PAD_PCT
    if len(centroids) > 0:
        in_bounds = (
            (centroids[:, 0] > w_img * m)
...
def extract_tiles_from_grid(
    ...
    Pipeline per tile:
    1. Snap crop center to nearest raw centroid (if provided) within tile_size/2.
    2. Take an inflated coarse crop (tile_size * 0.55 radius).
    3. Run Otsu + contour detection inside that crop to find the tight square bbox
       of the actual tile face.
    4. Return that tight crop (correct_tile_perspective then handles rotation).

    Args:
        return_bboxes: if True, returns (tiles, bboxes) where bboxes is a list of
            (x1, y1, x2, y2) tuples in warped-image coordinates.
    """
