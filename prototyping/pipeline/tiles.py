...
def correct_tile_perspective(tile_bgr):
    ...
    to axis-align it — matching the legacy per-tile warp_perspective_to_top_down().
    """
...
def preprocess_tile_v0(tile_bgr, target_size=TARGET_TILE_SIZE):
    ...
    Produces a target_size×target_size uint8 image (white letter on black background).
    This matches the preprocessing the BoggleCNN was trained on.

    Key details:
    - No CLAHE — matches training distribution (v0 used none)
    - 5% edge trim removes frame-bleed artifacts for border tiles
    - Adaptive threshold block_size = tile_area * 0.015 (>> tile width → effectively global)
    - Depth-1 contours filtered by min_area; depth-2+ kept unconditionally
    - Letter is centered in the original frame (preserving letter-to-canvas ratio)
    """
    gray = cv2.cvtColor(tile_bgr, cv2.COLOR_BGR2GRAY)
